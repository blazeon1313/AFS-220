...
def check_percentage(string, value):
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        executable_path = os.path.join(options['server_root'], 'apachectl')
        name = executable_path.ljust(len(options['process_name']))

        if sys.stdout.isatty() and not options['debug_mode']:
            print("Server Root Directory:", options['server_root'])
            process = subprocess.Popen([executable_path, 'start', '-DFOREGROUND'])

            process.wait()

        else:
            os.execl(executable_path, name, 'start', '-DFOREGROUND')
